...
//
// PLAN.md §4: the front end is a `nuxt generate` artefact served by `serve`,
// its API base is the relative path `/api`, and `scripts/check-static-bundle.mjs`
// fails the build if a hostname reaches the bundle. A bundle that carries no
// origin cannot notice which origin it was loaded from, so ONE container serves
// every scenario, each on its own published port, with no interference to
// arrange and no per-scenario build to pay for.
//
 ...
 * is examples/minimart-go-nuxt and the Dockerfile is <service>/Dockerfile.
 *
