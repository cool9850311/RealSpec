...
| For | Read |
|-----|------|
| Schema | `backend/migrations/000001_init.up.sql` |
...
| Configuration | `backend/internal/infrastructure/config/config.go`, `local/docker-compose.yml` |
...
- The front end is a static Nuxt SPA with three pages (`/login`, `/products`,
  `/orders`) in two locales (`zh_TW`, `en_US`). It talks to the API at the
...
`backend/migrations/000001_init.up.sql` — three tables, two indexes, and the
...
- **The `role` claim is not an authorisation decision.** Every endpoint that
  requires a role re-reads `users.role` for the subject in middleware, behind
...
- How the credential reaches the service is one thing, stated once: the `token`
  ...
  godog scenario writes `"headers": {"Cookie": "token={aliceToken}"}` into the
...
`OrderRepository.Redeem` (`backend/internal/infrastructure/repository/order_repo.go`),
...
`spec/` is the specification — this file, `openapi/`, `bdd/`. `backend/` is the
Go service (+ its godog runner under `src/test/`), `frontend/` the Nuxt SPA
(+ the Playwright suite under `tests/e2e/`).
...
|-------|------|----------|---------|
| Unit | `go test` | next to the code | Pure logic: JWT encode/decode, bcrypt verification, stock arithmetic, role enforcement |
| API (BDD) | godog | `spec/bdd/api/*.feature` | HTTP status, response body, and the resulting database state |
...
`.github/workflows/ci.yml` — three blocking stages in order: `realspec validate`, then `go test ./...` (unit + godog), then
`bddgen && playwright test`. Validation is first because it is the only stage
that costs nothing. Neither runner's concurrency is set there; both halve the
machine's CPU count.
...
|-------|-----------|----------|
| 1 — Infrastructure | — | Migrations, config, Gin router skeleton with `/api/v1` and CORS, and the godog harness (`scenario_ctx.go`: container lifecycle, migration runner, SQL helpers, an HTTP client with no cookie jar) |
| 2 — Auth | 1 | bcrypt + JWT issue (`POST /auth/login`), the JWT middleware, the role middleware that re-reads `users.role`, `GET /me`. Unblocks every authenticated scenario, because the features authenticate by actually logging in |
...
| 5 — Front end | the *contract* of 2–4, not their code | The three pages, `zh_TW` + `en_US`, and ARIA correctness — real roles and accessible names, `data-testid` only where no role and name identify the element. That is what makes the locator grammar sufficient |
...
`backend/internal/infrastructure/config/config.go` is the whole configuration
surface of the service — every name, its default, and why the migrations
directory and the front end's API base are deliberately not among them.
