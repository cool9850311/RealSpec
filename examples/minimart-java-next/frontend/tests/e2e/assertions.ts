 ...
 * (`document.documentElement.dataset.appReady`, set by the root layout's
 * Providers component once hydration is over and the post-hydration switch to
 * the visitor's locale has rendered — a test that raced it would compare the
 * screen against the wrong catalogue). The static export's HTML is prerendered
 * in en_US, so before that marker a zh_TW scenario is looking at English.
 */
