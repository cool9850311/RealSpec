...
//
// It is the browser-side counterpart of the `ScenarioContext` class in
// backend/src/test/java/minimart/bdd/ScenarioContext.java, and it holds the same
// four things that class holds — the context variable bag, the database handle,
// the record of what went over the wire, and the scenario's own addressing —
// plus the two a browser adds: the console log and the active locale's
// catalogue.
//
...
/**
 * The cookie the app reads its locale from after hydration, ahead of the
 * browser's languages (spec.md, "Deliberate differences from minimart-go-nuxt").
 * Setting it is what selects the locale: Chromium derives `navigator.languages`
 * from the context's locale option, not from the Accept-Language header below,
 * so the cookie is the one signal the Next.js app is guaranteed to see.
 */
...
export function watchBrowserErrors(page: Page): BrowserError[] {
  ...
  page.on('requestfailed', (request: Request) => {
    ...
    //
    // The Next.js static export adds a second source of the same event: its
    // link prefetch probes a route with a HEAD request, and Chromium reports
    // that HEAD as `net::ERR_ABORTED` even when the server answered 200 —
    // reproduced against a plain static file server with nothing of this app
    // in front of it. It is the same non-failure: an answer arrived, and the
    // page did not want its body.
    //
