...
export interface ScenarioStack {
  ...
  /** Applies backend/src/main/resources/db/migration to this scenario's database. */
...
/**
 * How long any one container may take to become ready.
 *
 * The binding case is the backend. A Spring Boot service is a JVM that loads,
 * links and JIT-compiles a framework before it opens its port — a few seconds
 * alone, and far more when every worker of a parallel run is booting one at once
 * on a two-vCPU runner. The go-nuxt harness's Go binary listened in milliseconds
 * under the same budget; this one needs the budget. The wait itself is still a
 * condition (the HTTP probe below), so a fast boot costs a fast boot — the number
 * only decides when a service that is not coming is declared dead.
 */
const STARTUP_TIMEOUT_MS = 120_000

/** How long the scenario's own origin may take to answer through Caddy. */
const ORIGIN_TIMEOUT_MS = 30_000

/**
 * The most a healthy startScenarioStack() can spend, stage by stage: PostgreSQL
 * and Caddy in parallel, then the backend, then the origin probe — each bounded
 * by its own timeout above — plus a margin for connecting to the database and
 * recording the ledger line.
 *
 * fixtures.ts gives the stack fixture exactly this budget. A smaller one would
 * let Playwright's generic "fixture timed out" pre-empt the failure that names
 * the container which did not come up; a larger one would only wait longer for
 * nothing, because by then some stage above has already thrown.
 */
export const STACK_STARTUP_BUDGET_MS = STARTUP_TIMEOUT_MS * 2 + ORIGIN_TIMEOUT_MS + 30_000
...
export async function startScenarioStack(profile: StackProfile): Promise<ScenarioStack> {
  ...
  try {
    ...
    // side effect of a container booting. The service's own Flyway run stays
    // off, so this database is exactly what migrationFiles() applies.
    const backend = await new GenericContainer(BACKEND_IMAGE)
      ...
      // the database is entitled to fail at this point. Spring Boot's embedded
      // Tomcat binds its port only once the application context has refreshed,
      // so an answer here also means the whole context — AppConfig's checks,
      // the connection pool, the security chain — came up.
    ...
    await waitForOrigin(baseURL, ORIGIN_TIMEOUT_MS)
...
/** Flyway's versioned-migration name: `V<version>__<description>.sql`. */
const FLYWAY_VERSIONED = /^V(\d+)__.+\.sql$/

/**
 * The migrations, in the order Flyway applies them.
 *
 * Numeric order, not lexical: Flyway compares versions as numbers, so V10 runs
 * after V9, and a string sort would put it before V2. A `.sql` file that is not
 * a versioned migration is an error rather than skipped — a repeatable (`R__`)
 * or misnamed script is one the service's Flyway would treat differently from
 * this loop, and a schema built two ways is not one schema. So is a version
 * claimed twice, which Flyway itself refuses.
 */
function migrationFiles(): string[] {
  const versioned: Array<{ version: number; name: string }> = []
  for (const name of fs.readdirSync(MIGRATIONS_DIR)) {
    if (!name.endsWith('.sql')) continue
    const match = FLYWAY_VERSIONED.exec(name)
    if (!match) {
      throw new Error(
        `run migration: ${name} in ${MIGRATIONS_DIR} is not a Flyway versioned migration ` +
          '(V<version>__<description>.sql); this harness applies only those',
      )
    }
    const version = Number(match[1])
    const clash = versioned.find((entry) => entry.version === version)
    if (clash) {
      throw new Error(`run migration: ${clash.name} and ${name} both claim version ${version}`)
    }
    versioned.push({ version, name })
  }
  if (versioned.length === 0) {
    throw new Error(`run migration: ${MIGRATIONS_DIR} holds no V<version>__<description>.sql file`)
  }
  return versioned
 ...
 */
async function waitForOrigin(baseURL: string, timeoutMs: number): Promise<void> {
